function [ stress, dis] = TenBarAnalysis( r, length, E, F )
...
for i=1:10
    ...
    length(i)=(n(ec(i,2),1)-n(ec(i,1),1))/length_e(i);
...
for i=1:10
    ...
    k=zeros(12,12);
    k_small = E*A(i)/length_e(i)*[length(i) m(i)]'*[length(i) m(i)];
...
dis = sqrt(Q(3)^2+Q(4)^2);
%% Stress Calculatuion
for i=1:10
    stress(i,:)=E/length_e(i,:)*[-length(i,:) -m(i,:) length(i,:) m(i,:)]...
